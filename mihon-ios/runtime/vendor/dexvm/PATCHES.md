...
- `vm/native/keiyoushi.rs`: HttpSource `getPopularManga` / `fetchPopularManga` defaults use `getClient().newCall().awaitSuccess()` so Cloudflare 403 is not parsed as an empty catalog. Default `*Request` joins `baseUrl` + path and forwards source headers.
