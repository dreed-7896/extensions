    ...
    fn step(&mut self, f: &mut Frame) -> Result<StepOutcome, JvmError> {
        ...
        let out = match insn {
            ...
            Insn::Cmp(kind, d, a, b) => {
                ...
                let c = match *kind {
                    CmpOp::CmpLong => match (as_i64(va), as_i64(vb)) {
                        (Some(x), Some(y)) => x.cmp(&y) as i32,
                        _ => 0,
                    },
                    CmpOp::CmpgFloat => match (as_f32(va), as_f32(vb)) {
                        (Some(x), Some(y)) => fcmp(x, y, true),
                        _ => 0,
                    },
                    CmpOp::CmplFloat => match (as_f32(va), as_f32(vb)) {
                        (Some(x), Some(y)) => fcmp(x, y, false),
                        _ => 0,
                    },
                    CmpOp::CmpgDouble => match (as_f64(va), as_f64(vb)) {
                        (Some(x), Some(y)) => fcmp(x, y, true),
                        _ => 0,
                    },
                    CmpOp::CmplDouble => match (as_f64(va), as_f64(vb)) {
                        (Some(x), Some(y)) => fcmp(x, y, false),
                        _ => 0,
                    },
                };
            ...
            Insn::If(op, a, b, t) => {
                ...
                let cond = match *op {
                    ...
                    IfOp::Lt => num_cmp(&va, &vb).map(|o| o.is_lt()).unwrap_or(false),
                    IfOp::Ge => num_cmp(&va, &vb).map(|o| o.is_ge()).unwrap_or(false),
                    IfOp::Gt => num_cmp(&va, &vb).map(|o| o.is_gt()).unwrap_or(false),
                    IfOp::Le => num_cmp(&va, &vb).map(|o| o.is_le()).unwrap_or(false),
                    _ => false,
                };
                if cond {
...
fn num_cmp(a: &JValue, b: &JValue) -> Option<std::cmp::Ordering> {
    match (a, b) {
...
fn coerce_arith(a: JValue, b: JValue) -> (JValue, JValue) {
    let rank = |v: JValue| match v {
        JValue::Double(_) => 3,
        JValue::Float(_) => 2,
        JValue::Long(_) => 1,
        JValue::Int(_) => 0,
    ...
    let conv = |v: JValue| match r {
        3 => as_f64(v).map(JValue::Double).unwrap_or(v),
        2 => as_f32(v).map(JValue::Float).unwrap_or(v),
        1 => as_i64(v).map(JValue::Long).unwrap_or(v),
        _ => as_i32(v).map(JValue::Int).unwrap_or(v),
    };
    (conv(a), conv(b))
}
