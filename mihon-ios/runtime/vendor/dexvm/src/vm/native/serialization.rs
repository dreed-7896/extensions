...
fn class_id_of(vm: &Vm, v: JValue) -> Option<u32> {
    match payload(vm, v) {
        Some(Native::ClassObj(ClassOrPrim::Class(id))) => Some(*id),
        _ => {
            if let JValue::Obj(o) = v {
                vm.arena.objects.get(o as usize).map(|obj| obj.class)
            } else {
                None
            }
        }
...
fn same_kclass(vm: &Vm, a: JValue, b: JValue) -> bool {
    a == b || matches!((class_id_of(vm, a), class_id_of(vm, b)), (Some(x), Some(y)) if x == y)
}

fn norm_poly_name(s: &str) -> String {
    s.rsplit(['.', '/', '$'])
        .next()
        .unwrap_or(s)
        .trim_matches(|c| c == 'L' || c == ';')
        .chars()
...
fn poly_name_match(serial: &str, disc: &str) -> bool {
    if serial == disc {
        return true;
    }
    let a = norm_poly_name(serial);
    let b = norm_poly_name(disc);
...
fn polymorphic_deserialize(vm: &mut Vm, args: &[JValue]) -> R {
    ...
    let mut matching_bases: Vec<(Vec<(JValue, JValue)>, Option<JValue>)> = Vec::new();
    let mut other_bases: Vec<(Vec<(JValue, JValue)>, Option<JValue>)> = Vec::new();
    for (poly_base, subs, default) in polys {
        if same_kclass(vm, poly_base, base) {
            matching_bases.push((subs, default));
        } else {
            other_bases.push((subs, default));
        }
    }
    let scan = |vm: &mut Vm,
                groups: Vec<(Vec<(JValue, JValue)>, Option<JValue>)>,
                fallback: &mut Option<JValue>,
                candidates: &mut Vec<String>|
     -> Result<Option<(JValue, JValue)>, NatErr> {
        for (subs, default) in groups {
            if fallback.is_none() {
                *fallback = default;
            }
            for (kclass, serializer) in subs {
                ...
                if poly_name_match(&name, &discriminator) {
                    return Ok(Some((kclass, serializer)));
                }
            }
        }
        Ok(None)
