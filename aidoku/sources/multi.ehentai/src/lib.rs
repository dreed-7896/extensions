...
impl Source for EHentai {
	...
	fn get_search_manga_list(
		...
		// E-Hentai treats punctuation in free-text terms inconsistently. In particular,
		// a period can turn an otherwise valid title search into an empty result set.
		// Treat periods as word separators, matching the behavior users get from the
		// website and other readers while leaving tag/filter syntax untouched.
		let mut query_str = query.unwrap_or_default().replace('.', " ");
