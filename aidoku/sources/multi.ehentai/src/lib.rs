...
impl Source for EHentai {
	...
	fn get_search_manga_list(
		...
		// Keep punctuation intact. E-Hentai title searches such as
		// `Hi.Seiki Nikuboketsu` require the period to match correctly.
		...
		// Toplists use the unmodified search query.
...
impl ListingProvider for EHentai {
	fn get_manga_list(&self, listing: Listing, page: i32) -> Result<MangaPageResult> {
		...
		let url = match listing.id.as_str() {
			"latest" => {
				if next_param_clean.is_empty() {
					format!("{base_url}/")
				} else {
					format!("{base_url}/?{next_param_clean}")
				}
			}
			...
			"watched" => {
				if next_param_clean.is_empty() {
					...
					format!("{base_url}/watched?{next_param_clean}")
				}
			}
